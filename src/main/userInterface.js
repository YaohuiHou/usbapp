...
function displayFile(file, files) {
  ...
  if (file.type == 'file') {
    displayIcon(clone, file.fileName, file)
  } else {
    clone.querySelector(
      'img'
...
// 判断类型
function filetypeFun(file) {
  let postFix = 'file'
  let fileType = mineType.lookup(file.path)
  if (fileType) {
    let fileInfo = fileType.split('/')
    let arr = [
      'bmp',
      'jpg',
      'jpeg',
